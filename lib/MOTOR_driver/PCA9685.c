...
#include "MOTOR_driver/Debug.h"  //DEBUG()

#include <math.h>   //floor()
#include <stdio.h>
#include <unistd.h>

static void PCA9685_WriteReg(I2CDevice *dev, UBYTE reg, UBYTE value) {
    i2c_write_byte(dev, reg, value);
}
 ...
 */
static void PCA9685_WriteByte(I2CDevice *dev, UBYTE reg, UBYTE value)
{
    i2c_write_byte(dev, reg, value);
}
 ...
 */
static UBYTE PCA9685_ReadByte(I2CDevice *dev, UBYTE reg) {
    UBYTE value = 0;
    i2c_read_byte(dev, reg, &value);
    return value;
}
 ...
 */
static void PCA9685_SetPWM(I2CDevice *dev, UBYTE channel, UWORD on, UWORD off) {
 ...
 * PCA9685_Init(0x40);
 */
void PCA9685_Init(I2CDevice *dev) {
    PCA9685_WriteReg(dev, MODE1, 0x00);
}
 ...
 */
void PCA9685_SetPWMFreq(I2CDevice *dev, UWORD freq) {
    ...
    UBYTE oldmode = PCA9685_ReadByte(dev, MODE1);
    UBYTE newmode = (oldmode & 0x7F) | 0x10; // sleep

    PCA9685_WriteByte(dev, MODE1, newmode); // go to sleep
    PCA9685_WriteByte(dev, PRESCALE, prescale); // set the prescaler
    PCA9685_WriteByte(dev, MODE1, oldmode);
    usleep(5 * 1000);
    PCA9685_WriteByte(dev, MODE1, oldmode | 0x80);  //  This sets the MODE1 register to turn on auto increment.
}
 ...
 */
void PCA9685_SetPwmDutyCycle(I2CDevice *dev, UBYTE channel, UWORD pulse) {
    if(pulse == 0){
        PCA9685_SetPWM(dev, channel, 0, 0);
        return;
    }
    ...
    PCA9685_SetPWM(dev, channel, 0, (pulse * 4096) / 100 - 1);
}
 ...
 */
void PCA9685_SetLevel(I2CDevice* dev, UBYTE channel, UWORD value) {
    if (value == 1)
        PCA9685_SetPWM(dev, channel, 0, 4095);
    else
        PCA9685_SetPWM(dev, channel, 0, 0);
}
